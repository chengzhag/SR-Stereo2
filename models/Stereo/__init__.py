...
def getModel(model, maxDisp, dispScale, cuda, half):
    if type(model) in (list, tuple):
        if model[0] in ('StereoDown',):
            stereo = getModel(model[1], maxDisp=maxDisp, dispScale=dispScale, cuda=cuda, half=half)
            return globals()[model[0]](stereo)
        elif model[0] in ('SRStereo',):
            sr = SR.getModel(model[1], cuda=cuda, half=half)
            stereo = getModel(('StereoDown', model[2]), maxDisp=maxDisp, dispScale=dispScale, cuda=cuda, half=half)
